...
- 专辑子页支持上传本地音频（MP3 等），底部播放器可真实播放、暂停、拖进度和切歌；JSON 备份 schema 升级到 5 并包含本地音频记录。
...
设计原图位于 [docs/design-target/originals](docs/design-target/originals)。AI 生成的收藏角氛围素材及提示词见 [素材说明](docs/design-target/ASSET_PROVENANCE.md)。支持本地上传音频并在底栏真实播放；未接入第三方流媒体。

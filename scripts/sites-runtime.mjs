...
export async function createRuntime({port=0,persist=false}={}) {
  ...
  const mf=new Miniflare({name:'kpop-collection',modules:true,scriptPath:path.join(root,'dist/server/index.js'),compatibilityDate:'2026-07-30',host:'127.0.0.1',port,bindings:{LOCAL_PREVIEW:'true',OWNER_EMAIL:'owner@example.test'},d1Databases:['DB'],r2Buckets:['BUCKET'],d1Persist:persist?path.join(root,'.sites-runtime/d1'):false,r2Persist:persist?path.join(root,'.sites-runtime/r2'):false,assets:{directory:path.join(root,'dist/client'),binding:'ASSETS',run_worker_first:true}});
  try {

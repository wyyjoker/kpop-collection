...
await build({entryPoints:[path.join(root,'sites/worker-entry.mjs')],outfile:path.join(dist,'server/index.js'),bundle:true,format:'esm',platform:'browser',target:'es2022',minify:true,loader:{'.sql':'text'},define:{'process':'undefined','globalThis.process':'undefined'},plugins:[{name:'exclude-sqljs-node-branches',setup(build){build.onResolve({filter:/^node:(fs|crypto)$/},args=>({path:args.path,namespace:'unreachable-node'}));build.onLoad({filter:/.*/,namespace:'unreachable-node'},()=>({contents:'throw new Error("Node-only SQL.js branch is disabled in Workers");',loader:'js'}));}}],logLevel:'info'});

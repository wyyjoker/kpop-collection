...
在设置 → 备份与恢复中，云端 JSON 和 SQLite 导出均包含数据库记录、个人资料、实物照片及描述，以及被引用的上传图片。JSON schema 4 兼容旧版 1/2/3。SQLite 使用纯 JavaScript SQLite 引擎打包，不依赖 Worker 本地文件系统或动态 WebAssembly 编译。

...
export const COLUMNS = {
  ...
  album_audios: ['id','album_id','title','src','note','created_at'],
};
...
export function validateSnapshot(snapshot) {
  ...
  if (snapshot.schema_version !== undefined && ![1,2,3,4,5].includes(snapshot.schema_version)) fail('不支持此备份版本。');
  const data = {};
  for (const [table, fields] of Object.entries(COLUMNS)) {
    const optional = table==='album_photos' ? !(snapshot.schema_version>=4) : table==='album_audios' ? !(snapshot.schema_version>=5) : ['album_tracks','catalog_imports'].includes(table) && !(snapshot.schema_version >= 2);
    ...
    data[table] = rows.map(row => {
      ...
      if(table==='album_audios'&&(!/^\/uploads\/[a-zA-Z0-9_.-]+$/.test(out.src)||!out.title||out.title.length>200||out.note.length>2000))fail('本地音频格式无效。');
      return out;
    });
    const ids = new Set();
  ...
  if((data.album_audios??[]).some(r=>!albumIds.has(r.album_id)))fail('本地音频所属专辑不存在。');
...
export async function exportData(db) {
  ...
  return { format:'kpop-collection-backup',schema_version:5,app_version:'0.7-sites',exported_at:new Date().toISOString(),data };
}
export function assetReferences(data) {
  return new Set([...data.groups.flatMap(r=>[r.cover,r.logo]),...data.albums.map(r=>r.cover),...data.album_versions.map(r=>r.cover),...(data.album_photos??[]).map(r=>r.src),...(data.album_audios??[]).map(r=>r.src),data.profile.avatar,data.profile.hero_cover].filter(v=>v?.startsWith('/uploads/')));
}
